...
function initTeamOperatingCore() {
    ...
    const centralCore = document.getElementById('cinematic-central-core');
    ...
    if (svg) {
        ...
        nodes.forEach((node) => {
            ...
            line.setAttribute('data-member', node.getAttribute('data-member'));
            line.setAttribute('stroke', 'rgba(0, 240, 255, 0.15)');
            line.setAttribute('stroke-width', '1.5');
            line.style.transition = 'stroke 0.6s ease, stroke-width 0.6s ease, opacity 0.6s ease';
            svg.appendChild(line);
        });
    }

    const lines = svg ? svg.querySelectorAll('.network-line') : [];

    function updateLayout() {
        if (window.innerWidth <= 768) {
            // Mobile Mode: clear inline positioning and SVG lines
            nodes.forEach(node => {
                node.removeAttribute('style');
            });
            if (svg) svg.style.display = 'none';
            if (centralCore) centralCore.style.display = 'none';
            infoPanel.removeAttribute('style');
            infoWrappers.forEach(w => w.removeAttribute('style'));
            return;
        }

        // Desktop Mode: Center calculations
        if (svg) svg.style.display = 'block';
        if (centralCore) centralCore.style.display = 'flex';

        const rect = container.getBoundingClientRect();
        cx = rect.width / 2;
        cy = rect.height / 2;

        // Visual radius of the network
        const maxRadius = Math.min(rect.width * 0.36, 230);
        rx = maxRadius;
        ry = maxRadius;

        // Position nodes and update lines
        nodes.forEach((node, index) => {
            ...
            node.style.top = `${y}px`;
            node.style.transform = 'translate(-50%, -50%)'; // Center on coordinate
            node.style.transition = 'transform 0.6s cubic-bezier(0.16, 1, 0.3, 1), opacity 0.6s cubic-bezier(0.16, 1, 0.3, 1), filter 0.6s cubic-bezier(0.16, 1, 0.3, 1)';
            node.style.opacity = '0.85';

            // Sync SVG line positions
            if (lines[index]) {
                lines[index].setAttribute('x1', cx);
                lines[index].setAttribute('y1', cy);
    ...
    // Active hovered state handling
    nodes.forEach((node, index) => {
        const key = node.getAttribute('data-member');
        const info = Array.from(infoWrappers).find(w => w.getAttribute('data-member') === key);
        ...
        node.addEventListener('mouseenter', () => {
            ...
            nodes.forEach((n, idx) => {
                if (idx === index) {
                    n.style.transform = 'translate(-50%, -50%) scale(1.15)'; // 15% enlargement
                    n.style.opacity = '1.0';
                    n.classList.add('active-preview');
                } else {
                    n.style.transform = 'translate(-50%, -50%) scale(0.9)';
                    n.style.opacity = '0.4'; // fade out inactive
                    n.classList.remove('active-preview');
                }
            });
            ...
            // 3. Central Core Fades Out
            if (centralCore) {
                centralCore.style.opacity = '0';
                centralCore.style.pointerEvents = 'none';
            }

            // 4. Fade in central info panel
            infoPanel.setAttribute('data-active', key);
            infoPanel.style.opacity = '1';
            infoPanel.style.pointerEvents = 'auto';

            infoWrappers.forEach(w => {
                if (w === info) {
                    w.style.opacity = '1';
                    ...
                    w.style.opacity = '0';
                    w.style.transform = 'translate(-50%, -50%) translateY(12px)';
        ...
        node.addEventListener('mouseleave', () => {
            ...
            // Restore Central Core
            if (centralCore) {
                centralCore.style.opacity = '1';
                centralCore.style.pointerEvents = 'auto';
            }

            // Hide info panel
            infoPanel.removeAttribute('data-active');
